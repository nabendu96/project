...
def get_taus(filename, n_pixels, n_los):
    ...
    with open(filename, 'rb') as f:
        for i in range(n_los):
            ...
            tau = np.fromfile(f, dtype=np.float64, count=512)
            taus[:,i] = tau
            taus[:,i] -= taus[:,i].min()
            ...
            for k in range(n_pixels):
                ...
                dft[k,i]=s
            vaxis = velscale*np.arange(0,512.)/512.
            dx = vaxis[1] - vaxis[0]
            dft = dx*dft/velscale
            ...
            """
            temp = np.fromfile(f, dtype=np.float64, count=512)
            vpec = np.fromfile(f, dtype=np.float64, count=512)
            rho = np.fromfile(f, dtype=np.float64, count=512)
            rhoneutral = np.fromfile(f, dtype=np.float64, count=512)
            nhi = np.fromfile(f, dtype=np.float64, count=512)
            denstemp = np.fromfile(f, dtype=np.float64, count=512)
            n2 = np.fromfile(f, dtype=np.int64, count=1)
            assert(n==n2)

    return P,velscale

P,velscale = get_taus('./spec_z3.000_0_512.dat', 512, 5000)
#P1,velscale = get_taus('./spec_z3.000_1.dat', 256, 5000,1)
"""
f = open('Parr_0_512.txt', 'w')
n_pixels = 512
